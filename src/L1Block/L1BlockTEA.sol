...
contract L1Block is TeaWAPOracle, ISemver, IGasToken {
    ...
    function getL1DataCost(uint256, uint256, uint256 fastLzSize, bool isDepositTx) external view returns (uint256) {
        ...
        // todo: think about max calldata passed nad if it's reasonable, breaks things, etc.
        uint256 estimatedETHCost = _calculateL1DataCostFjord(fastLzSize);

        (uint256 nativeTokenPerETH, uint8 oracleDecimals) = _getNativeTokenPerETH();

        return estimatedETHCost * nativeTokenPerETH / (10 ** oracleDecimals);
    }
    ...
    function _calculateL1DataCostFjord(uint256 fastLzSize) internal view returns (uint256) {
        ...
        // todo: on the geth side, do we need this cdGasUsed?
